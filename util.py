...
def load_idxfile(filename):
    ...
    with open('data/' + filename,'rb') as _file:
...
def histogram_weights(weights, bins=100):
    plt.figure()
    plt.hist(np.reshape(weights, (-1, 1)), bins)
...
def histogram_hidden_probs(prob_h, bins=50):
    prob_h = np.mean(prob_h, axis=0)    # get the mean of each unit across all samples
    plt.figure()
    plt.title('Mean activation prob of hidden units')
    plt.xlabel('Probability')
    plt.ylabel('Number of hidden units')
    plt.hist(np.reshape(prob_h, (-1, 1)), bins)
    plt.show()

def plot_recons_error(error, xlabel=None):
    plt.plot(error)
